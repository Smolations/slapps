...
class NectarAppNotifyListener extends JenkinsNotifyListener {
  get name() {
    return Registry.for(this).get('NectarSubscriber').jobNames.APP;
  }
  ...
  startMessage(notification) {
    return `Nectar app deploy started...`;
  }
  ...
  async successMessage(notification) {
    ...
    return [
      `Nectar app deployed! Deploy duration: *${duration.pretty}*`,
  ...
  failureMessage(notification) {
    ...
    return [
      `Oh noes! The app deploy failed. Here's an excerpt from the <${build.full_url}/console|Console>:`,
      '```',
      build.log,
      '```',
    ].join('\n');
...
module.exports = NectarAppNotifyListener;
