...
class NectarThemeNotifyListener extends JenkinsNotifyListener {
  get name() {
    return Registry.for(this).get('NectarSubscriber').jobNames.THEME;
  }
  ...
  startMessage(notification) {
    return `Nectar theme deploy started...`;
  }
  ...
  async successMessage(notification) {
    ...
    return [
      `Nectar theme deployed! Deploy duration: *${duration.pretty}*`,
  ...
  failureMessage(notification) {
    ...
    return [
      `Oh noes! The theme deploy failed. Here's an excerpt from the <${build.full_url}/console|Console>:`,
      '```',
      build.log,
      '```',
    ].join('\n');
...
module.exports = NectarThemeNotifyListener;
